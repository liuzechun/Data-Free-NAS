...
def get_args():
    ...
    parser.add_argument('--val-interval', type=int, default=5000, help='report frequency')
...
def main():
    ...
    # load teacher model
    model_teacher = torchvision.models.__dict__['resnet50'](pretrained=True)
    ...
    if use_gpu:
        ...
        loss_function_ce = criterion_smooth.cuda()
        loss_function = criterion_kd.cuda()
        device = torch.device("cuda")
    else:
        loss_function_ce = criterion_smooth
        loss_function = criterion_kd
    ...
    args.loss_function_ce = loss_function_ce
    ...
    while all_iters < args.total_iters:
        all_iters = train(model, model_teacher, device, args, val_interval=args.val_interval, bn_process=False, all_iters=all_iters)
...
def train(model, model_teacher, device, args, *, val_interval, bn_process=False, all_iters=None):
    ...
    for iters in range(1, val_interval + 1):
        ...
        loss = loss_function(output, output_teacher)
...
def validate(model, device, args, *, all_iters=None):
    ...
    loss_function = args.loss_function_ce
